...
+ (NSURL *)URLforPhotosInPlace:(id)flickrPlaceId onPage:(int)onPage perPage:(int)perPage;
{
    /*return [self URLForQuery:[NSString stringWithFormat:@"https://api.flickr.com/services/rest/?method=flickr.photos.search&place_id=%@&page=%d&per_page=%d&extras=original_format,tags,description,geo,date_upload,owner_name,place_url", flickrPlaceId,onPage,perPage]];*/
...
+ (NSString *)urlStringForPhoto:(NSDictionary *)photo format:(FlickrPhotoFormat)format
{
	...
	switch (format) {
        ...
        case FlickrPhotoFormatNormal:    formatString = @"m"; break;
	}
